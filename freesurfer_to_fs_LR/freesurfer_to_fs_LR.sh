...
hemi=$3
#CaretAtlasDirectory=/mnt/myelin/donna/ATLAS/fs_LR/standard_mesh_atlases
CaretAtlasDirectory=$2/freesurfer_to_fs_LR/standard_mesh_atlases

#FreeSurferInstallationDirectory=/usr/local/freesurfer
FreeSurferInstallationDirectory=/Applications/freesurfer
OutputRootDir=fs_LR_output
...
# Adding the cut topoloy to spec file modified by Rouhi
  cp "$CaretAtlasDirectory"/fsaverage.$Hemisphere.cut_cartesian-standard.164k_fs_LR.topo $OutputRootDir/$Subject/$Subject.$Hemisphere.cut_cartesian-standard.164k_fs_LR.topo.gii
  cp "$CaretAtlasDirectory"/fsaverage.$Hemisphere.cartesian-std.164k_fs_LR.coord $OutputRootDir/$Subject/$Subject.$Hemisphere.cartesian-std.164k_fs_LR.coord.gii
  cp "$CaretAtlasDirectory"/fsaverage.$Hemisphere.sulc.164k_fs_LR.shape.gii $OutputRootDir/$Subject/fsaverage.$Hemisphere.sulc.164k_fs_LR.shape.gii
  caret_command -spec-file-add $OutputRootDir/$Subject/$InitialMeshDirectory/$Subject.$Hemisphere.mni+orig.initial_mesh.spec CUTtopo_file $Subject.$Hemisphere.cut_cartesian-standard.164k_fs_LR.topo.gii
  caret_command -spec-file-add $OutputRootDir/$Subject/$InitialMeshDirectory/$Subject.$Hemisphere.mni+orig.initial_mesh.spec FLATcoord_file $Subject.$Hemisphere.cartesian-std.164k_fs_LR.coord.gii
  caret_command -spec-file-add $OutputRootDir/$Subject/$InitialMeshDirectory/$Subject.$Hemisphere.mni+orig.initial_mesh.spec surface_shape_file fsaverage.$Hemisphere.sulc.164k_fs_LR.shape.gii
# I have to add template or fsaverage sulci pattern to the folder of subjects and to the scene file
# end of modification
